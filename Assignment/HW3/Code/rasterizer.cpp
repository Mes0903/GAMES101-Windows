...
namespace {
bool insideTriangle(float x, float y, Eigen::Vector4f const (&_v)[3]) {
	Vector3f v[3];
...
std::tuple<float, float, float> computeBarycentric2D(float x, float y, Eigen::Vector4f const (&v)[3]) {
...
rst::col_buf_id rst::rasterizer::load_normals(std::vector<Eigen::Vector3f> const& normals) {
	auto id = get_next_id();
...
void rst::rasterizer::draw(std::vector<Triangle*>& TriangleList) {
	...
	for (auto const& t : TriangleList) {
		...
		Eigen::Vector4f n[] = {inv_trans * to_vec4(t->normal[0], 0.0f), inv_trans * to_vec4(t->normal[1], 0.0f), inv_trans * to_vec4(t->normal[2], 0.0f)};
...
static Eigen::Vector3f interpolate(float alpha, float beta, float gamma, Eigen::Vector3f const& vert1, Eigen::Vector3f const& vert2,
								   Eigen::Vector3f const& vert3, float weight) {
	return (alpha * vert1 + beta * vert2 + gamma * vert3) / weight;
}

static Eigen::Vector2f interpolate(float alpha, float beta, float gamma, Eigen::Vector2f const& vert1, Eigen::Vector2f const& vert2,
								   Eigen::Vector2f const& vert3, float weight) {
...
void rst::rasterizer::rasterize_triangle(Triangle const& t, std::array<Eigen::Vector3f, 3> const& view_pos) {
		...
		for (int y = min_y; y < max_y; ++y) {
			if (insideTriangle(x + 0.5, y + 0.5, t.v)) {
				...
				if (z_interpolated < depth_buf[pixel_index]) {
					...
					frame_buf[pixel_index] = pixel_color;
				}
			}
		}
	}
...
void rst::rasterizer::set_pixel(Vector2i const& point, Eigen::Vector3f const& color) {
	// old index: auto ind = point.y() + point.x() * width;
	int ind = (height - point.y()) * width + point.x();
	frame_buf[ind] = color;
}

void rst::rasterizer::set_vertex_shader(std::function<Eigen::Vector3f(vertex_shader_payload)> vert_shader) { vertex_shader = vert_shader; }

void rst::rasterizer::set_fragment_shader(std::function<Eigen::Vector3f(fragment_shader_payload)> frag_shader) { fragment_shader = frag_shader; }
